...
#  library(package = "rnaturalearthdata"); # for getting coord data
  ...
  statesSF = st_as_sf(states);
  statesSF2 = st_as_sf(states, crs = 2955);
  statesSF3 = st_as_sf(states, crs = 26917);
  ...
  ### What is the CRS for the geom_sf?
  crs1 = st_crs(statesSF);
  crs2 = st_crs(statesSF2);
  crs3 = st_crs(statesSF3);
  ...
  plot2 = ggplot() +
    ...
    geom_sf(data = statesSF,
            ...
            color = "black", 
            fill = "grey") +
    # this works if everything has a defined crs
    coord_sf(xlim = c(-130, -60),  # in meters or feet?
             ylim = c(25, 50),
             expand = TRUE);
  plot(plot2);
  ...
  plot3 = ggplot() +
    ...
    coord_sf(crs = 6135, #4269,   # NAD 83 - geodetic 
             xlim = c(-130, -60),  # in meters or feet?
             ylim = c(25, 50),
             expand = FALSE);
  plot(plot3);
