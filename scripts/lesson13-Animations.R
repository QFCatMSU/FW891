{ 
  ...
  timeStamp_Orig = fishTracks_SF$bin_timestamp;
  timeStamp_Date = as.Date(timeStamp_Orig, format="%m/%d/%Y");
  timeStamp_POSIXlt = as.POSIXlt(timeStamp_Orig,format="%m/%d/%Y %H:%M",tz=Sys.timezone());
  timeStamp_POSIXct = as.POSIXct(timeStamp_Orig,format="%m/%d/%Y %H:%M",tz=Sys.timezone());
  ...
  plot3 = plot1 +
    coord_sf(crs = 26917,   # UTM 17N
        ...
        expand = TRUE) +  # Add padding to the plot
        labs(title = "Fish Date: {frame_time}") +
        transition_time(time=timeStamp_POSIXct);
  # plot(plot3);
  ...
  plot4 = plot1 +
    coord_sf(crs = 26917,   # UTM 17N
        ...
        transition_states(states = timeStamp_POSIXct,
                  transition_length = 1,
                  state_length = 2,
                  wrap = TRUE);
  # plot(plot4);
  ...
  anim_save(filename = "images/animate-timeCT-100.gif",   # have to use CT
            animation = plot3,
            nframes = 100,       # number of frames in animation
            fps = 10);           # frames per second
  
  anim_save(filename = "images/animate-timeCT-Range.gif",   # have to use CT
            animation = plot3,
            nframes = 100,       # number of frames in animation
            fps = 10);           # frames per second
  
  # anim_save(filename = "images/animate-stateCT.gif",
  #           animation = plot4,
